...
- Before doing anything, it is recommended to backup your current Meshtastic node config.
- From your phone app while connected to your Meshtastic node, go to Settings > Export Configuration, select all toggles, and then save that .cfg file somewhere safe.
...
- Just like when doing firmware updates via serial, you're going to want a good cable that enables data transmission (many cables are charge-only).
    - If you have a cable that worked for Meshtastic flashing, it should work for MeshCore as well.
...
## 3. Connect your node to your computer

If it's USB-A, it's upside-down, flip it. Now flip it again.
GUESS WHAT, IT WAS RIGHT THE FIRST TIME, flip it one more time.

## 4. Open the MeshCore Flasher

Go to [https://flasher.meshcore.io/](https://flasher.meshcore.io/)

Note that you'll need to use a Chromium-based browser, like Chrome or Edge.
Firefox isn't going to work.

## 5. Select Device Type

Find your exact node (exact model) and click on it.

If you can't find your exact model, ask in the Discord.
...
- If you have a multi-node setup
    - Select "Companion Bluetooth" for your pocket node.
    - Select "Repeater" for the powerful/stationary node (the node on your roof, in your tree, in your attic, etc.)
        - Note that repeater nodes cannot be configured via bluetooth, but once configured over USB you can make changes from your companion node.

## 7. Flash the MeshCore Firmware
...
- Watch the progress. Beware that erasing flash may take some time (minutes) where it appears little progress is occurring, then it will then progress to flashing. 

**DON'T CLOSE THE WINDOW OR DISCONNECT THE DEVICE YET**

## 8. Configure your new MeshCore Node

### For Companion Nodes
- After flashing in step 7, disconnect the node from your computer and pair it to your phone using Bluetooth.
    - If the device has a screen, the PIN is displayed there as a randomized six digit number.
    - If the device has no screen, 123456 is the default PIN.
- Configure the radio to match the settings listed in [our post about "MeshCore500"](/2026/09/02/fcc-regulations/#meshcore-and-meshtastic-fcc-compliant-settings)

### For Repeater Nodes
- After flashing in step 7, DO NOT DISCONNECT the node from your computer. Initial settings must be made over serial first!
...
- The repeater setup page has a few settings you MUST setup before you disconnect, we'll start with those:
    ...
    - Under RADIO SETTINGS, change the settings to match the settings listed in [our post about "MeshCore500"](/2026/09/02/fcc-regulations/#meshcore-and-meshtastic-fcc-compliant-settings). These settings (freq, bandwidth, SF, coding rate) must match your companion node for the two to see each other and the rest of the mesh. 
    - Click SAVE SETTINGS at the bottom. A toast message with "Data successfully saved" should pop up for a second.
- At this point you can disconnect your repeater and configure the rest over LoRa (using the password you just set), or finish setting it up here on a bigger screen while you're already connected.
...
- Your repeater should now be automatically repeating packets, and when you send a message using your companion node, you should see your repeater as the first hop out.

### Update Time and Location
Look for a setting similar to "sync/set device clock to phone time" and click it.
- MeshCore can be weird with time settings, it is a good practice to occasionally do this so your node's clock doesn't become too out-of-sync with the mesh.
- Some apps have a setting to sync automatically.

Set a location for your node, especially if it's a repeater.
- The easiest way to do this is from the map view, where you can select a location and set it as your node location.
- Note that the location does not get "fuzzed" like in Meshtastic, please do not use your exact home address if you are not comfortable doing so.
